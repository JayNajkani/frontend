...
const EPUser = () =>{
  ...
  useEffect(()=>{
    ...
    axios.get(_userapiurl+"fetch?email="+fetchemail).then((response)=>{
  ...
  const handleSubmit=()=>{
    let updateDetails = {"condition_obj":{"email":email},"content_obj":{
      "name":name,"mobile":mobile,"address":address,"city":city,"gender":gender}};
    axios.patch(_userapiurl+"update",updateDetails).then((response)=>{
      setOutput("Details updated successfully");
